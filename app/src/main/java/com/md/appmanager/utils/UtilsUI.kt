...
//        }
        ...
        drawerBuilder.withOnDrawerItemClickListener { view, position, iDrawerItem ->
            when (iDrawerItem.identifier) {
                1L -> recyclerView.adapter = appAdapter
                2L -> recyclerView.adapter = appSystemAdapter
                3L -> recyclerView.adapter = appFavoriteAdapter
                4L -> recyclerView.adapter = appHiddenAdapter
                5L -> UtilsDialog.showProFeatures(context)
                6L -> context.startActivity(Intent(context, SettingsActivity::class.java).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
                7L -> context.startActivity(Intent(context, AboutActivity::class.java).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK))
                else -> {
                }
            }
            false
